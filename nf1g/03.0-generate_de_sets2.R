...
#genotype names. 
sample_info$resultant_geno%>%unique()

#get genos of interest for filtering.
genotypes_of_interest<-c("nf1 KO; pten KO; ink KO; atrx KO", 
             "nf1 KO; pten KO; ink KO; atrx wt")


data_filtered<-read_counts%>%left_join(sample_info)%>%ungroup()%>%arrange(sample_id)%>%
  filter(resultant_geno %in% genotypes_of_interest)
...
categories<-c("patho_cat_name", "patho_cat2_name", "patho_cat_det_name", "patho_grade")
...
ga<-genotypes_of_interest[1]
gb<-genotypes_of_interest[2]


group_a_count<-comp_info%>%
  filter(resultant_geno==!!ga)%>%count()%>%pull()


group_b_count<-comp_info%>%
  filter(resultant_geno==!!gb)%>%
  count()%>%pull()
...
comp_counts<-all_data%>%
  ...
  c_group<-comp_info%>%
    select(all_of(ct), resultant_geno)%>%
    mutate(model_bin=if_else(resultant_geno==ga, 0, 
                             if_else(resultant_geno==gb, 1, NA_real_)))%>%
    pull(model_bin)
  ...
  #save output file.
  saveRDS(output2, paste0("nf1g/dexps/atrx_comps/", "dexp-", fs::path_sanitize(paste0(category, "-", value, "-", ga,  " v. ", gb)), ".rds"))
