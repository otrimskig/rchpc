...
for (c in 1:length(column_subset)){


anno_block<-anno_df1%>%filter(patho_cat_name==column_subset[c])
...
assign(paste0("hm_",c),Heatmap(gz_sub1_block,
...
comb_hm<-hm_1 + hm_2 + hm_3 + hm_4 + hm_5 + ha1 + rowAnnotation(rn = anno_text(rownames(gz_sub1_block), 
...
hm_pdf_name<-paste0("nf1g/plots/gsva_hms/hm-disease-groups-", analysis_name, "-segment-", seg_num, ".pdf")
