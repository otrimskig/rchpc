...
file_name_addition<-"_tumortype_"


#get paths of rds file analyses.
a<-dir_ls("nf1g/gsvas/ds")%>%
  tibble(path=.)%>%
  filter(grepl("_stats_nf1 ", path))
...
group_comparison_name<-sub(" :.*","",analysis_list$metadata$group_comparison)
...
df1 <- df0 %>%
  filter(
    min_pval <= quantile(min_pval, 0.04, na.rm = TRUE),  # lowest 10% p-values
    abs_diff >= quantile(abs_diff, 0.98, na.rm = TRUE)   # top 10% abs_diff
  )%>%
  filter(min_pval<=.05)
