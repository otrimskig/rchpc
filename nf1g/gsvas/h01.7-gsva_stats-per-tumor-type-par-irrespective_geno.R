...
if(length(group_a_samples)<2|length(group_b_samples)<2){
  ...
  analysis_output <- list(
    ...
    metadata = list(
      date = Sys.time(),
      group_comparison = paste("all genos: ", group_factors[1], "vs", group_factors[2]),
  ...
  num_rows <- nrow(sub_matrix)
  
  
  # Initialize vectors to store results
  p_values_ttest <- numeric(num_rows)
  p_values_lm <- numeric(num_rows)
  fold_changes <- numeric(num_rows)
  analysis_method <- character(num_rows)
  mean_a <- numeric(num_rows)
  mean_b <- numeric(num_rows)
  mean_total <- numeric(num_rows)
  std_a <- numeric(num_rows)
  std_b <- numeric(num_rows)
  
  
  # export_vars <- c("sub_matrix", "group_a_matrix", "group_b_matrix", "group_factors", 
  #                "valid_a_samples", "valid_b_samples", "nrow")
  
  pb <- txtProgressBar(max = num_rows, style = 3)
  progress <- function(n) setTxtProgressBar(pb, n)
  opts <- list(progress = progress)
  
  results <- foreach(i = 1:num_rows, .combine = rbind, 
                     .options.snow = opts, .packages = "stats") %dopar% {
    source("libs.R")
    ...
    # Fit linear model
    lm_model <- lm(z_score ~ Group, data = lm_data)
    p_lm <- summary(lm_model)$coefficients[2, 4]
    fold_change <- coef(lm_model)[2]
    
    # Determine analysis method
    method <- ifelse(p_lm < p_ttest, "lm", "t-test")
    
    # Compute statistics
    mean_a_val <- mean(a_values, na.rm = TRUE)
    mean_b_val <- mean(b_values, na.rm = TRUE)
    mean_total_val <- mean(combined_values, na.rm = TRUE)
    std_a_val <- sd(a_values, na.rm = TRUE)
    std_b_val <- sd(b_values, na.rm = TRUE)
    
    
    # # Progress tracking
    # if (i %% n_progress == 0) {
    #   cat("d2: ", i, "out of", num_rows, " ", round(i/num_rows*100, 1), "%", "\n")
    # }
    
    # Return results as a row
    c(p_ttest, p_lm, fold_change, method, mean_a_val, mean_b_val, mean_total_val, std_a_val, std_b_val)
  }
  ...
  p_values_ttest <- results[,1]
  p_values_lm <- results[,2]
  fold_changes <- results[,3]
  analysis_method <- results[,4]
  mean_a <- results[,5]
  mean_b <- results[,6]
  mean_total <- results[,7]
  std_a <- results[,8]
  std_b <- results[,9]
  ...
  results <- tibble(
    Pathway = rownames(sub_matrix),
    p_value_ttest = as.numeric(p_values_ttest),
    p_value_lm = as.numeric(p_values_lm),
    fold_change = as.numeric(fold_changes),
    mean_total = as.numeric(mean_total),
    mean_group_a = as.numeric(mean_a),
    mean_group_b = as.numeric(mean_b),
    std_group_a = as.numeric(std_a),
    std_group_b = as.numeric(std_b),
    analysis_method = as.character(analysis_method),  # Ensure it's a character vector
    diff = as.numeric(as.numeric(mean_b) - as.numeric(mean_a)),
    abs_diff = abs(as.numeric(mean_b) - as.numeric(mean_a))  # Explicit numeric conversion
  ) %>%
    mutate(min_pval = pmin(p_value_lm, p_value_ttest))
