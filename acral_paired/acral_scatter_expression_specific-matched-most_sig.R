...
genes_list<-gene_sigs%>%
  arrange(PValue)%>%
  slice(1:100)%>%
  pull(gene_name_ms)

goi1<-goi0%>%
  filter(gene_name_ms %in% genes_list)
...
goi<-goi1%>%arrange(PValue)%>%pull(gene_name_ms)
...
plot_list[[g]]<-ggplot(df1, aes(x=sample_type, y=rpkm))+
  ...
  labs(title=paste0(x_label, "\n", 
                    "p = ", pval),
       x="")+
  # annotate(
  #   "text", 
  #   label = paste0(),
  #   x = , y = 15, size = 5, colour = "black"
  # )+
...
ggsave(paste0("acral_paired/plots/", "combined_individual_genes_dot-paired-top_100_sig",".pdf"),
