...
all_dexps<-list.files("k19mf/dexps", full.names = T, pattern="^dexp.*\\.rds$", recursive = F)

#get sample metadata.
all_sample_info2<-readRDS("nf1g/ds/v07-per_sample_info.rds")
...
# anno_color<-readRDS("nf1g/ds/hm_colors_list.rds")
...
anno<-HeatmapAnnotation(df=de_samples%>%select(tumor_type, aod), 
                        
                        # col=c(anno_color, aod=aod_colors),
                        
                        col=c(tumor_type=c("darkgreen", "purple"), aod=aod_colors),
...
ggsave(paste0("k19mf/plots/",fs::path_sanitize(paste0("hm-", file_base, "-rpkm.pdf"))),
       plot=gh,
...
plots<-list.files("k19mf/plots", full.names = T, pattern="^hm.*\\.pdf$")



qpdf::pdf_combine(plots, output = "k19mf/plots/combined/chm-1.pdf")
