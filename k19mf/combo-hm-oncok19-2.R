...
keep_mouse_nums<-de_samples%>%
  filter(tumor_type=="foot pad")%>%
  pull(mouse_num)
...
for (colnum in 1:length(de_anno_colnames)){
  ...
  unique_to_filter<-de_anno_df%>%select(!!sym(colname))%>%unique()%>%
    filter(tumor_type=="foot pad")%>%pull()%>%as.character()
...
gsva_subset<-gsva_u%>%as.data.frame()%>%
  ...
  select(all_of(c("pathway", keep_mouse_nums)))%>%
  column_to_rownames("pathway")%>%
  as.matrix.data.frame()
...
ggsave(paste0("k19mf/plots/hm-pathways-unsc-fponly-pvsubset01-u", "-clustered-relative-summary-stats-no-aod.pdf"),
       plot=gh2,
